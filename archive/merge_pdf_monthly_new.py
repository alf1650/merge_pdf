...
def get_base_block(block):
    # Extract just the numeric part (first sequence of digits)
        ...
        for i in range(total_pdf_pages):
            ...
            if i < len(pages_list):
                ...
                if "blocks" not in page_info:
                    ...
                    for block in blocks:
                        found = False
                        # First try exact match with full block name
                        full_key = (equipment, block)
                        matched_images = images_by_equip_block.get(full_key, [])
                        ...
                        if matched_images:
                            for img_path in matched_images:
                                ...
                                print(f"    ➕ Adding image: {os.path.basename(img_path)} (for block {block})")
                                ...
                                if found:
                                    break  # Processed one image for this block
                        else:
                            # If no exact match, try with just numeric base
                            base_block = get_base_block(block)
                            if base_block != block:  # Only try base if it's different from original
                                base_key = (equipment, base_block)
                                matched_images = images_by_equip_block.get(base_key, [])
                                ...
                                if matched_images:
                                    for img_path in matched_images:
                                        ...
                                        print(f"    ➕ Adding image: {os.path.basename(img_path)} (for block {block}, using base {base_block})")
                        ...
                        if not found:
                            print(f"    ➖ No image found for block {block}")
